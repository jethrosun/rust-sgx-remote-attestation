...
(cd ra-enclave && cargo +nightly-2020-05-30 build --target x86_64-fortanix-unknown-sgx -Zfeatures=itarget --example tls-enclave --features example) && \
...
# Build client
(cd ra-client && cargo +nightly-2020-05-30 build -Zfeatures=itarget --example tls-client --features verbose)

# Build SP
(cd ra-sp && cargo +nightly-2020-05-30 build -Zfeatures=itarget --example tls-sp --features "verbose")
